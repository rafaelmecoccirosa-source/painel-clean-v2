...
import { MOCK_CLIENTE, MOCK_INDICACOES, initialsOf, formatDateBR } from '@/lib/mock-cliente';

const LINK = 'painelclean.com.br/ref/FA2026';
...
export default function IndicacoesView() {
  ...
  const progresso = (c.indicacoesAtivas / 5) * 100;
  const descontoReal = Math.round((c.mensalidadeOriginal * c.descontoIndicacao) / 100);
    ...
    <main style={{ maxWidth: 1200, margin: '0 auto', padding: '32px 28px 72px', display: 'grid', gap: 24 }}>
        ...
        >
          <div>
            ...
            <p style={{ fontSize: 15, color: 'rgba(255,255,255,.75)', marginTop: 10 }}>
              {c.indicacoesAtivas} indicações ativas · R$ {descontoReal},00/mês de desconto
            </p>
          </div>
          <div>
            <div
            ...
            <div style={{ marginTop: 8, fontSize: 13, color: 'rgba(255,255,255,.85)', fontWeight: 600 }}>
              {c.indicacoesAtivas} de 5 indicações · {5 - c.indicacoesAtivas} faltam
            </div>
          </div>
        </div>
      </section>
        ...
        {NIVEIS.map((n) => {
          ...
          return (
            <div
              key={n.n}
              style={{
                background: ativo ? COLORS.light : 'white',
                border: `${ativo ? 2 : 1}px solid ${ativo ? COLORS.green : COLORS.border}`,
                borderRadius: 14,
                padding: 18,
                textAlign: 'center',
                boxShadow: ativo ? '0 4px 14px rgba(61,196,90,.15)' : '0 2px 12px rgba(27,58,45,.04)',
              }}
            >
              <div
                style={{
                  fontSize: 11,
                  fontWeight: 700,
                  color: ativo ? COLORS.dark : COLORS.muted,
                  textTransform: 'uppercase',
                  letterSpacing: '.08em',
                  marginBottom: 10,
                }}
      ...
      >
        ...
        <p style={{ fontSize: 12, color: COLORS.muted, margin: 0, lineHeight: 1.55 }}>
          Quando seu amigo assinar usando esse link, você recebe +6% de desconto por 12 meses e ele ganha 50% off na 1ª
          limpeza.
        </p>
      </section>
          ...
          {MOCK_INDICACOES.map((row, i) => (
            <div
              ...
              style={{
                ...
                alignItems: 'center',
                padding: '16px 22px',
                borderBottom: i < MOCK_INDICACOES.length - 1 ? `1px solid ${COLORS.border}` : 'none',
              }}
            >
              <div
                style={{
              ...
              <div>
                ...
                <div style={{ fontSize: 12, color: COLORS.muted, marginTop: 3 }}>
                  Indicado em {formatDateBR(row.data)} · Expira {row.expira}
                </div>
              </div>
              ...
              <span
                style={{
                  ...
                  fontWeight: 700,
                  fontSize: 16,
                  color: row.status === 'ativo' ? COLORS.green : COLORS.muted,
                }}
              >
                {row.desconto}
              </span>
